...
This appendix aims to illustrate the process of calibrating five variants of an
age-structured SEIR model to synthetic data via the Nelder-Mead algorithm and 
Hamiltonian Monte Carlo. The first section shows the inputs and outputs of
the procedure of synthetic data generation. The second section presents the 
...
## Average contacts per age group per day
...
cpt <- "Finland's daily average conversational contacts by age group. 
Source: POLYMOD"
...
Let $m'_{ij}$ define the mean number of contacts per time unit from 
one individual in age class i with individuals in age class j reported in the
POLYMOD's study. Due to variation in the sample from which the contact matrix is
obtained, the reciprocity condition -*the total number of contacts from age 
class $j$ to age class $i$ must be equal to the total number of contacts from 
age class $i$ to age class $j$*- is not fulfilled exactly. For this reason, we 
correct the age-specific contact rate for reciprocity ($m_{ij}$) from the average
of the total number of contacts between the two age classes:

\begin{equation}
 m_{ij} = \frac{m'_{ij} N_j + m'_{ji} N_i}{2 N_j}
\end{equation}

By doing so, we satisfy:
...
```{r, fig.align= "center"}
cpt <- "Finland's daily average conversational contacts disaggregated in four age cohorts. The raw data has been\ncorrected for reciprocity following equation 1. Source: POLYMOD"
...
##  Synthetic WAIFW (B) & synthetic scaled WAIFW (K)

```{r g_synWAIFW, fig.height = 4.5, fig.width = 7, fig.align= "center"}

cpt <- "Left matrix: Rates at which two specific individuals come into effective contact by age group.\n  Also known as the WAIFW matrix.\nRight matrix: WAIFW matrix scaled up by the population size (N = 10,000)."
...
g_syn_WAIFW <- output_sp$g_syn_WAIFW +
  theme(plot.margin = unit(c(1, 1, -2, 1), "cm"))

g_K_matrix <- output_sp$g_K_matrix +
  theme(plot.margin = unit(c(1, 1, -2, 1), "cm"))

ggarrange(g_syn_WAIFW, g_K_matrix,
          bottom = text_grob(cpt, hjust = 0, size = 10, x = 0, face = "italic"))
```
...
cpt <- "True daily new cases of influenza by age group"
fc  <- fc + 1
cpt <- paste0("Figure ", fc, ". ", cpt)
...
cpt <- "Reported daily new cases of influenza by age group and scenario."

fc  <- fc + 1
cpt <- paste0("Figure ", fc, ". ", cpt)

g_timeseries + labs(caption = cpt) +
  theme(plot.caption = element_text(hjust = 0, face= "italic"),
        plot.caption.position =  "plot")

ggsave("./plots/A_incidence_reports.pdf", plot = g_timeseries ,dpi = "print", 
       height = 5, width = 8)
```
...
matrix of size four, equation 3 defines the number of possible permutations. 
...
cpt <- 
  "Variants of scaled WAIFW matrices. \n Source: Kannan & Farrington (2005)"

fc  <- fc + 1
cpt <- paste0("Figure ", fc, ". ", cpt)


g_matrices <- grid.arrange(g_cm[[1]], g_cm[[2]], g_cm[[3]], g_cm[[4]], 
                           g_cm[[5]], ncol = 2,
                           bottom = text_grob(cpt, hjust = 0, size = 10, x = 0, face = "italic"))
...
n_inits     <- 24
plot_titles <- paste0("Init id = ", 1:n_inits)
...
cpt <- 
  "Calibration of SEIR A via Nelder-Mead to the perfect information incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."

fc  <- fc + 1
cpt <- paste0("Figure ", fc, ". ", cpt)

tg <- text_grob(cpt, hjust = 0, size = 6, x = 0, face = "italic")
do.call("grid.arrange", list(grobs = comparison_graphs, ncol = 2, bottom = tg))
```

```{r, fig.height = 7}
cpt <- 
  "Estimated scaled WAIFW matrices from SEIR A. Each panel shows the results for the six initialisations with the highest likelihood. \nThe title of each panel corresponds to the initialisation id."
...
tg       <- text_grob(cpt, hjust = 0, size = 6, x = 0, face = "italic")
do.call("grid.arrange", list(grobs = K_matrix, ncol = 2, bottom = tg))
```
\newpage
...
cpt <- "Accuracy measurements of the estimated incidences from SEIR A's six initialisations with the \nhighest likelihood."
...
g_errors <- draw_inits_comparison(top_optim_A, output_sp$theoretical_R0, 
                                top_id) 
cpt <- "Accuracy of the epidemiological quantities (scaled WAIFW matrix, basic reproduction number)\nestimated from SEIR A's six initialisations with the highest likelihood. Text within the parentheses indicates the\nspecific accuracy measure."
...
cpt <- 
  "Calibration of SEIR B via Nelder-Mead to the perfect information incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."
...
```{r, fig.height = 7}
cpt <- 
  "Estimated scaled WAIFW matrices from SEIR B. Each panel shows the results for the six initialisations with the highest likelihood. \nThe title of each panel corresponds to the initialisation id."
...
cpt <- "Accuracy measurements of the estimated incidences from SEIR B's six initialisations with the \nhighest likelihood."
...
g_errors <- draw_inits_comparison(top_optim_B, output_sp$theoretical_R0, 
                                top_id) 
cpt <- "Accuracy of the epidemiological quantities (scaled WAIFW matrix, basic reproduction number)\nestimated from SEIR B's six initialisations with the highest likelihood. Text within the parentheses indicates the\nspecific accuracy measure."
...
cpt <- 
  "Calibration of SEIR C via Nelder-Mead to the perfect information incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."
...
```{r, fig.height = 7}
cpt <- 
  "Estimated scaled WAIFW matrices from SEIR C. Each panel shows the results for the six initialisations with the highest likelihood. \nThe title of each panel corresponds to the initialisation id."
...
cpt <- "Accuracy measurements of the estimated incidences from SEIR C's six initialisations with the \nhighest likelihood."
...
g_errors <- draw_inits_comparison(top_optim_C, output_sp$theoretical_R0, 
                                top_id) 
cpt <- "Accuracy of the epidemiological quantities (scaled WAIFW matrix, basic reproduction number)\nestimated from SEIR C's six initialisations with the highest likelihood. Text within the parentheses indicates the\nspecific accuracy measure."
...
cpt <- 
  "Calibration of SEIR D via Nelder-Mead to the perfect information incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."
...
g_for_plot <- do.call("arrangeGrob", c(comparison_graphs, ncol = 2))
ggsave(file = "./plots/A_matrix_D_ts_optim.pdf", g_for_plot, dpi = "print",
       height = 7, width = 5)
```

```{r, fig.height = 7}
cpt <- 
  "Estimated scaled WAIFW matrices from SEIR D. Each panel shows the results for the six initialisations with the highest likelihood. \nThe title of each panel corresponds to the initialisation id."
...
cpt <- "Accuracy measurements of the estimated incidences from SEIR D's six initialisations with the \nhighest likelihood."
...
g_errors <- draw_inits_comparison(top_optim_D, output_sp$theoretical_R0, 
                                top_id) 
cpt <- "Accuracy of the epidemiological quantities (scaled WAIFW matrix, basic reproduction number)\nestimated from SEIR D's six initialisations with the highest likelihood. Text within the parentheses indicates the\nspecific accuracy measure."
...
cpt <- 
  "Calibration of SEIR S via Nelder-Mead to the perfect information incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."
...
```{r, fig.height = 7}
cpt <- 
  "Estimated scaled WAIFW matrices from SEIR S. Each panel shows the results for the six initialisations with the highest likelihood. \nThe title of each panel corresponds to the initialisation id."
...
cpt <- "Accuracy measurements of the estimated incidences from SEIR S's six initialisations with the \nhighest likelihood."
...
cpt <- "Accuracy of the epidemiological quantities (scaled WAIFW matrix, basic reproduction number)\nestimated from SEIR S's six initialisations with the highest likelihood. Text within the parentheses indicates the\nspecific accuracy measure."
...
cpt <- "Comparison between the average (mean) simulated transmission rates obtained from the calibration\nof the SEIR A via HMC, and the actual scaled WAIFW values.\nValues inside the brackets indicate 95 % credible intervals."
...
```{r}
cpt <- "Posterior distribution estimated from SEIR A. The diagonal shows the posterior marginal\ndistributions. In the lower triangular part, it is presented the joint posterior distribution of each possible\ncombination of two parameters. The upper triangular part shows the correlation among parameters."
...
```{r g_ts_stan_A, fig.align = 'center'}
cpt <- "SEIR A's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
cpt <- "Predicted basic reproduction number from the calibration of SEIR A."
...
cpt <- "Comparison between the average (mean) simulated transmission rates obtained from the calibration\nof the SEIR B via HMC, and the actual scaled WAIFW values.\nValues inside the brackets indicate 95 % credible intervals."
...
```{r}
cpt <- "Posterior distribution estimated from SEIR B. The diagonal shows the posterior marginal\ndistributions. In the lower triangular part, it is presented the joint posterior distribution of each possible\ncombination of two parameters. The upper triangular part shows the correlation among parameters."
...
```{r g_ts_stan_B, fig.align = 'center'}
cpt <- "SEIR B's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
cpt <- "Predicted basic reproduction number from the calibration of SEIR B."
...
cpt <- "Comparison between the average (mean) simulated transmission rates obtained from the calibration\nof the SEIR C via HMC, and the actual scaled WAIFW values.\nValues inside the brackets indicate 95 % credible intervals."
...
```{r}
cpt <- "Posterior distribution estimated from SEIR C. The diagonal shows the posterior marginal\ndistributions. In the lower triangular part, it is presented the joint posterior distribution of each possible\ncombination of two parameters. The upper triangular part shows the correlation among parameters."
...
```{r g_ts_stan_C, fig.align = 'center'}
cpt <- "SEIR C's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
cpt <- "Predicted basic reproduction number from the calibration of SEIR C."
...
cpt <- "Comparison between the average (mean) simulated transmission rates obtained from the calibration\nof the SEIR D via HMC, and the actual scaled WAIFW values.\nValues inside the brackets indicate 95 % credible intervals."
...
```{r}
cpt <- "Posterior distribution estimated from SEIR D. The diagonal shows the posterior marginal\ndistributions. In the lower triangular part, it is presented the joint posterior distribution of each possible\ncombination of two parameters. The upper triangular part shows the correlation among parameters."
...
```{r g_ts_stan_D, fig.align = 'center'}
cpt <- "SEIR D's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
cpt <- "Predicted basic reproduction number from the calibration of SEIR D."
...
cpt <- "SEIR D's trace plot. Panels corresponds to the four separate parameters in this configuration.\nEach paneldisplays four Markov chains. The shaded area indicates the warm-up period."  
...
ggsave(file = "./plots/A_traceplot_D.pdf", g_for_plot, dpi = "print",
       height = 4, width = 6)
```
\newpage
...
specs_list <- list(x_pos = 2.31, ypos_mean = 30, ypos_median = 26,
                   ...
                   title = "Symmetric matrix - R0", xlabel = "")
...
g_for_plot <- arrangeGrob(pred_K, grey_actual_K, ncol = 2)

ggsave(file = "./plots/A_WAIFW_sym_HMC.pdf", plot = g_for_plot, dpi = "print",
       height = 3, width = 6)

cpt <- "Comparison between the average (mean) simulated transmission rates obtained from the calibration\nof the SEIR S via HMC, and the actual scaled WAIFW values.\nValues inside the brackets indicate 95 % credible intervals."
...
```{r}
cpt <- "Posterior distribution estimated from SEIR S. The diagonal shows the posterior marginal\ndistributions. In the lower triangular part, it is presented the joint posterior distribution of each possible\ncombination of two parameters. The upper triangular part shows the correlation among parameters."
...
```{r g_ts_stan_S, fig.align = 'center'}
cpt <- "SEIR S's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
cpt <- "Predicted basic reproduction number from the calibration of SEIR S."
...
g <- ggplot(error_ts, aes(x = matrix, y = MASE)) +
  geom_lollipop(aes(colour = is_min)) +
  scale_colour_manual(values = c("grey", "steelblue")) +
  coord_flip() +
  geom_text(aes(label = round(MASE, 2), colour = is_min), nudge_y = 0.2, 
            size = 3) +
  facet_wrap(~ method) +
  labs(x = "Variant") +
  theme_test() +
...
ggsave("./plots/A_incidence_fit_perfect_info.pdf", plot = g, dpi = "print",
       height = 3, width = 8)

cpt <- "Accuracy of estimated incidences by model variant and calibration method."
...
g <- ggplot(log_liks_df , aes(x = matrix, y = log_lik)) +
...
cpt <- "Log-likelihood by model variant and calibration method. For HMC, displayed values correspond\nto mean estimates from Stan's samples."
...
cpt <- 
  "Calibration of SEIR A via Nelder-Mead to the underreported incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."
...
cpt <- 
  "Calibration of SEIR B via Nelder-Mead to the underreported incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."
...
cpt <- 
  "Calibration of SEIR C via Nelder-Mead to the underreported incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."
...
cpt <- 
  "Calibration of SEIR D via Nelder-Mead to the underreported incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."
...
comparison_graphs <- purrr::map(top_optim_sym, "g_comparison")
cpt <- 
  "Calibration of SEIR S via Nelder-Mead to the underreported incidence. Each panel shows the results for the six initialisations with the highest likelihood. \nIn these panels, the results are disaggregated by age. Grey lines represent the synthetic data. Blue lines represent the incidence estimated by the fitting model.\nThe title of each panel corresponds to the initialisation id and the basic reproductive number calculated from the calibration."
...
```{r g_ts_stan_und_A, fig.align = 'center'}
cpt <- "SEIR A's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
```{r g_ts_stan_und_B, fig.align = 'center'}
cpt <- "SEIR B's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
```{r g_ts_stan_und_C, fig.align = 'center'}
cpt <- "SEIR C's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
```{r, fig.height = 2.5, fig.width = 5.5, fig.align = 'center'}
specs_list <- list(x_pos = 2.33, ypos_mean = 20, ypos_median = 17,
                   text_size = 2, ypos_interval = 14,
                   title = "Matrix D - R0", xlabel = "")
...
```{r g_ts_stan_und_D, fig.align = 'center'}
cpt <- "SEIR D's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
```{r g_ts_stan_und_sym, fig.align = 'center'}
cpt <- "SEIR S's incidence fit. The blue line indicates the average time-series estimated from Stan's\nsamples. The shaded area indicates the 95 % credible interval. The grey dots denote the actual data."
...
ggsave("./plots/A_incidence_fit_underreporting.pdf", plot = g, dpi = "print", 
       height = 3, width = 8)
...
ggsave("./plots/A_comparison_reporting_value.pdf", dpi = "print", height = 3,
       width = 8)

cpt <- "Estimated reporting probabilities by model variant and calibration method.\nThe dashed line denotes the actual value. Bars indicate the 95 % credible interval in HMC estimates."

fc  <- fc + 1
cpt <- paste0("Figure ", fc, ". ", cpt)

g_comparison_reporting_value +
labs(caption = cpt) +
theme(plot.caption = element_text(hjust = 0, face= "italic"),
      plot.caption.position =  "plot")
```
...
g <- ggplot(R_nought_est, aes(x = matrix, y = R0)) +
  geom_point() +
  ...
  labs(x = "Variant", y = bquote(R[0]))

ggsave("./plots/A_R0comparison.pdf", plot = g, dpi = "print", height = 5,
       width = 8)

cpt <- "Estimated basic reproduction number by model variant and calibration method.\nThe dashed line denotes the actual value. Bars indicate the 95 % credible interval in HMC estimates."
...
g <- ggplot(K_error_df, aes(x = matrix, y = SMAPE)) +
  geom_lollipop(aes(colour = is_min)) +
  scale_y_continuous(limits = c(0, 2)) +
  coord_flip() +
  geom_text(aes(label = round(SMAPE, 2),
                colour = is_min), nudge_y = 0.2, size = 3) +
  scale_colour_manual(values = c("grey", "steelblue")) +
...
ggsave("./plots/A_WAIFWcomparison.pdf", plot = g, dpi = "print", height = 5,
       width = 8)

cpt <- "Accuracy of estimated scaled WAIFW matrices by model variant and calibration method."
...
cpt <- "Computational time spent in fitting each model variant by calibration method."
