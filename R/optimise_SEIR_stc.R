optimise_SEIR_stc <- function(matrix_type, data_list, init.WAIFW = 20) {
  ...
  poisson.loglik <-  function (params) {
    ...
    SEIR_params    <- params[1:length(params) - 1]
    sim_incidences <- get_incidence(SEIR_params)
    
    lik1 <- -1 * sum(dpois(lambda = (sim_incidences$incidence1 + 0.0000001) * p, x = age_0_4_data, 
                           log = TRUE))
    
    lik2 <- -1 * sum(dpois(lambda = (sim_incidences$incidence2 + 0.0000001) * p, x = age_5_14_data, 
                           log = TRUE))
    
    lik3 <- -1 * sum(dpois(lambda = (sim_incidences$incidence3 + 0.0000001) * p, x = age_15_44_data, 
                           log = TRUE))
    
    lik4 <- -1 * sum(dpois(lambda = (sim_incidences$incidence4 + 0.0000001) * p , x = age_45_over_data, 
                           log = TRUE))
    
    sum(lik1, lik2, lik3, lik4)
  }
  
  optim_fit <- optim(par = c(rep(init.WAIFW, length(params)), 0.5), 
                     fn = poisson.loglik, 
                     method = "Nelder-Mead")
  
  names(optim_fit$par) <- c(params, "p")
